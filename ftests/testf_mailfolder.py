...
class MailFolderTest(FunctionalTestCase):
    ...
    def testAddMailBox(self):
        ...
        response = self.publish( '/%s/manage_addProduct/CPSMailAccess/' \
           'manage_addMailFolder' % folder_name,
        ...
        self.assertEqual(response.getHeader('location'),
                         'http://nohost/%s/my_folder/manage_main' % folder_name)

def test_suite():
    return unittest.TestSuite((
        unittest.makeSuite(MailFolderTest),
        ))
